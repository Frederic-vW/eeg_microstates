...
def kmeans(data, n_maps, n_runs=10, maxerr=1e-6, maxiter=500, doplot=False):
	...
	while ( (np.abs((var0-var1)/var0) > maxerr) & (n_iter < maxiter) ):
	    ...
	    n_iter += 1
	if (n_iter < maxiter):
	    print("\t\tK-means run {:d}/{:d} converged after {:d} iterations."\
	    	  .format(run+1, n_runs, n_iter))
	else:
    ...
    if doplot:
        ...
        for imap in range(n_maps):
            axarr[imap].imshow(eeg2map(maps[imap, :]), cmap=cm)
...
def testMarkov1(X, ns, alpha, verbose=True):
    ...
    for t in range(n-2):
        ...
        f_j[j] += 1.0
    T = 0.0
    for i, j, j in np.ndindex(f_ijk.shape):
...
def geoTest(X, ns, dt, alpha, verbose=True):
    ...
    for s in range(ns): # test for each symbol
        ...
        g1, p1, dof1, expctd1 = \
        chi2_contingency( np.vstack((q_obs,q_exp)), lambda_='log-likelihood' )
        if verbose:
            print("\t\tG-test (log-likelihood) p: {:.2e}, g: {:.3f}, df: {:.1f}".\
                  format(p1,g1,dof1))
        
        g2, p2, dof2, expctd2 = \
        chi2_contingency( np.vstack((q_obs,q_exp)) ) # Pearson's Chi2 test
        if verbose:
            print("\t\tG-test (Pearson Chi2) p: {:.2e}, g: {:.3f}, df: {:.1f}".\
                  format(p2,g2,dof2))
        ...
        fig.patch.set_facecolor('white')
        times_exp = ( 1 + np.arange(len(q_exp)) ) * dt * 1000
        times_obs = ( 1 + np.arange(len(q_obs)) ) * dt * 1000
        plt.semilogy(times_exp,q_exp,'-b',linewidth=5,alpha=1.0)
        plt.semilogy(times_obs,q_obs,'ok',markersize=10,alpha=0.8)
        plt.xlabel('duration [ms]')
        plt.ylabel('probability')
        plt.title("Lifetimes-%d: GeoTest p=%4.3f, mean: %3.2f ms".\
                  format(s,p1,tau_mean))
        plt.show()
        '''
    return p_values
...
def multiple_comparisons(p_values, method):
    ...
    multipletests(p_values, method=method)
...
def main():
    ...
    parser.add_argument('-i','--input', help='.edf file name', \
    			required=False)
    parser.add_argument('-f','--filelist', help='.edf file list', \
    			required=False)
    parser.add_argument('-d', '--directory', help='process all .edf files '\
    			'in this directory', required=False)
    parser.add_argument('-m', '--markovsurrogates', help='number of ' \
    			'Markov surrogates', required=False)
	...
	os.system("clear")
	print("\n\t[4] Run modified K-means clustering:")
	n_maps = 4
	maps, x, gfp_peaks, gev, cv = \
	kmeans(data, n_maps=n_maps, n_runs=5, doplot=True)
	...
	print("\t\tGlobal explained variance GEV = {:.2f}"\
	      .format(gev.sum()))
	raw_input("\n\n\t\tpress ENTER to continue...")
	...
	#os.system("clear")
	print("\n\t[6] Shannon entropy of the microstate sequence:")
	h_hat = H_1(x, n_maps)
	...
	os.system("clear")
	print("\n\t[14] Mutual information of " + \
	      "EEG microstates and surrogate MC:\n")
	l_max = 100
	#print(argsdict)
