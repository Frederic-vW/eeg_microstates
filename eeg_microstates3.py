...
def clustering(data, fs, chs, locs, mode, n_clusters, n_win=3, doplot=False):
...
def aahc(data, N_clusters, doplot=False):
    ...
    while (n_maps > N_clusters):
        blank_ = 80*" "
        s = f"\r{blank_:s}\r\t\tAAHC > n: {n_maps:d} => {n_maps-1:d}"
...
def print_matrix(T):
    ...
    for i, j in np.ndindex(T.shape):
        if (j == 0):
            ...
            #print "{:.3f}|\n".format(T[i,j]), # Python-2
            print("{:.3f}|\n".format(T[i,j]), end='') # Python-3
        else:
            #print "{:.3f}".format(T[i,j]), # Python-2
            print("{:.3f}".format(T[i,j]), end='') # Python-3
...
def main():
    ...
    for filename in filenames:
        ...
        for ch in chs:
            #print(type(ch), ch)
            #print "\t{:s}".format(ch), # Python-2
            print("\t{:s}".format(ch.decode()), end='') # Python-3
        input("\n\n\n\t\tpress ENTER to continue...")
        ...
        mode = ["aahc", "kmeans", "kmedoids", "pca", "ica"][3]
